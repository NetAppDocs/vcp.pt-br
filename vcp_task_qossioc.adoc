...
NOTE: Para implantações do NetApp HCI, a senha padrão é gerada aleatoriamente durante a instalação. Para determinar a senha, consulte o procedimento 4 neste  https://kb.netapp.com/on-prem/solidfire/Element_OS_Kbs/mNode_Status_Network_Down_or_Down_in_the_mNode_Settings_tab_of_Element_Plugin_for_vCenter_VCP["KB"^] artigo.
...
NOTE: Consulte isto https://kb.netapp.com/on-prem/solidfire/Element_OS_Kbs/mNode_Status_Network_Down_or_Down_in_the_mNode_Settings_tab_of_Element_Plugin_for_vCenter_VCP["KB"^] para solucionar problemas caso o status seja um dos seguintes: * `Down`: QoSSIOC não está habilitado. * `Not Configured`: As configurações de QoSSIOC não foram configuradas. * `Network Down`: vCenter não consegue se comunicar com o serviço QoSSIOC na rede. O mNode e o serviço SIOC podem ainda estar em execução.
