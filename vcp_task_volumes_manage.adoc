...
Os sistemas operacionais diferem na forma como tratam volumes clonados. O ESXi tratará um volume clonado como uma cópia de volume ou um volume de snapshot. O volume será um dispositivo disponível para você usar na criação de um novo datastore. Para mais informações sobre como montar volumes clonados e lidar com LUNs de snapshot, consulte a documentação da VMware sobre https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/7-0/vsphere-storage/working-with-datastores-in-vsphere-storage-environment/vsphere-vmfs-datastore-copies-and-datastore-resignaturing/resignature-a-vmfs-datastore-copy-in-the-vsphere-web-client.html["Montagem de uma cópia do datastore VMFS"^] e https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/7-0/vsphere-storage/working-with-datastores-in-vsphere-storage-environment/vsphere-vmfs-datastore-copies-and-datastore-resignaturing.html["Gerenciando armazenamentos de dados VMFS duplicados"^].
